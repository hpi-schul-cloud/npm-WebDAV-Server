	...
	test('events', (isValid, server) => {
		...
		function expectEvents(testName, events, callback) {
			...
			function check() {
				...
				while (ev.length > 0) {
					...
					if (index === -1) return '[' + testName + '] Missing event "' + e + '"';
			...
			callback((path, method, headers, body) => {
				req(path, method, headers, body, (res) => {
					if (Math.floor(res.statusCode / 100) !== 2)
						isValid(
							false,
							'[' + testName + '] Received status code : ' + res.statusCode + ' (' + res.statusMessage + ')'
						);
					else {
						const result = check();
		...
		req('/test4.txt', 'PUT', undefined, 'BODY', () => {
			req('/test4.copy.txt', 'PUT', undefined, 'BODY', () => {
				expectEvents(
					'COPY to an existing resource',
					['copy', 'create', 'read', 'addChild', 'delete', 'write'],
					(req) => {
		...
		req('/test10.txt', 'PUT', undefined, 'BODY', () => {
			expectEvents('PROPPATCH set property', ['setProperty', 'setProperty'], (_, check) => {
				req(
					...
					'PROPPATCH',
					undefined,
					'<?xml version="1.0" encoding="utf-8" ?><D:propertyupdate xmlns:D="DAV:" xmlns:Z="http://ns.example.com/standards/z39.50/"><D:set><D:prop><Z:Authors><Z:Author>Jim Whitehead</Z:Author><Z:Author>Roy Fielding</Z:Author></Z:Authors><Z:Authors2><Z:Author>Jim Whitehead</Z:Author><Z:Author>Roy Fielding</Z:Author></Z:Authors2></D:prop></D:set></D:propertyupdate>',
					() => {
						const result = check();
						if (result) {
							isValid(false, result);
						...
						expectEvents('PROPPATCH remove property', ['removeProperty', 'removeProperty'], (req) => {
							req(
								...
								'PROPPATCH',
								undefined,
								'<?xml version="1.0" encoding="utf-8" ?><D:propertyupdate xmlns:D="DAV:" xmlns:Z="http://ns.example.com/standards/z39.50/"><D:remove><D:prop><Z:Authors/><Z:Authors2/></D:prop></D:remove></D:propertyupdate>'
							);
						});
					}
				);
