...
module.exports = (test, options, index) =>
	test('PROPPATCH method', (isValid, server) => {
		...
		function testGroup(prefix, isJSON) {
			server.rootResource.addChild(
				new webdav.VirtualFile(prefix + 'testFile.txt'),
				test(prefix + 'testFile.txt', isJSON)
			);
			server.rootResource.addChild(
				new webdav.VirtualFolder(prefix + 'testFolder'),
				test(prefix + 'testFolder', isJSON)
			);

			const pFileName = prefix + 'testPFile.txt';
			const pFilePath = path.join(__dirname, 'proppatch', pFileName);
			if (!fs.existsSync(pFilePath)) fs.writeFileSync(pFilePath, 'Content!');
			...
			if (!fs.existsSync(pFolderPath)) fs.writeFileSync(pFolderPath, 'Content!');
					...
					request(
						{
							...
							method: 'PROPPATCH',
							body:
								'<?xml version="1.0" encoding="utf-8" ?><D:propertyupdate xmlns:D="DAV:" xmlns:Z="http://ns.example.com/standards/z39.50/"><D:set><D:prop><Z:Authors><Z:Author>Jim Whitehead</Z:Author><Z:Author>Roy Fielding</Z:Author></Z:Authors></D:prop></D:set></D:propertyupdate>',
							headers: {
										...
										(e, res, body) =>
											_(e, () => {
												tryCatch(() => {
													const xml = isJSON
														? JSON.parse(body)
														: xmljs.xml2js(body, { compact: true, alwaysArray: true });
													...
													request(
														{
															...
															method: 'PROPPATCH',
															body:
																'<?xml version="1.0" encoding="utf-8" ?><D:propertyupdate xmlns:D="DAV:" xmlns:Z="http://ns.example.com/standards/z39.50/"><D:remove><D:prop><Z:Authors/></D:prop></D:remove></D:propertyupdate>',
															headers: {
																Accept: isJSON ? 'application/json' : undefined,
															},
														},
																		...
																		_(e, () => {
																			tryCatch(() => {
																				const xml = isJSON
																					...
																					: xmljs.xml2js(body, { compact: true, alwaysArray: true });
																				const prop =
																					xml['D:multistatus'][0]['D:response'][0]['D:propstat'][0]['D:prop'][0];
																				const authorsKey = Object.keys(prop).find((k) => k.endsWith(':Authors'));
