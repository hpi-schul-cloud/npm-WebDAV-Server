...
module.exports = (test, options, index) =>
	test('move a physical resource', (isValid, server) => {
		...
		function test(prefixName, isFile, constructor) {
			...
			if (fs.existsSync(filePathDest)) {
				if (isFile) fs.unlinkSync(filePathDest);
				else fs.rmdirSync(filePathDest);
			}
			if (!fs.existsSync(filePath)) {
				if (isFile) fs.writeFileSync(filePath, 'Content!');
				else fs.mkdirSync(filePath);
			}

			const groupResource = new webdav.PhysicalFolder(groupFolderPath);
			server.rootResource.addChild(groupResource, (e) =>
				_(e, () => {
					groupResource.addChild(constructor(filePath), (e) =>
						_(e, () => {
									...
									exist('/' + groupFolder + '/' + fileNameDest, (exists) => {
										...
										fs.exists(filePath, (exists) => {
											...
											fs.exists(filePathDest, (exists) => {
												isValid(
													exists,
													'The ' + type + ' must be present in the real destination, but it is not the case [p -> p]'
												);
											});
										});
									});
								...
								exist('/' + fileName, (exists) => {
									...
									exist('/' + fileNameDest + '/' + fileName, (exists) => {
										...
										fs.exists(filePath, (exists) => {
											isValid(
												exists,
												'The ' + type + ' must not be really moved when moved to a virtual folder [p -> v]'
											);
										});
									});
								});
