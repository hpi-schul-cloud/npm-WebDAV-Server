...
module.exports = (test, options, index) =>
	test('resource tester on the virtual stored resources', (isValid, server) => {
		isValid = isValid.multiple(2, server);

		const storePath = path.join(__dirname, 'resourceTesterVirtualStored');
		for (const file of fs.readdirSync(storePath)) if (file !== '.gitkeep') fs.unlinkSync(path.join(storePath, file));
