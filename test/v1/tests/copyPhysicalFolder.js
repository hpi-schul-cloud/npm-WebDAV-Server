...
module.exports = (test, options, index) =>
	test('copy a physical folder', (isValid, server) => {
		...
		if (!fs.existsSync(subFilePath)) fs.writeFileSync(subFilePath, 'Content!');
		...
		function rmDir(dirPath) {
			fs.readdirSync(dirPath).forEach((name) => {
				...
				if (fs.statSync(childPath).isFile()) fs.unlinkSync(childPath);
				else rmDir(childPath);
			});
		...
		if (fs.existsSync(destFolderPath)) rmDir(destFolderPath);
