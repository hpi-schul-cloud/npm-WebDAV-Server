...
function go(
	...
	range: string,
	callback: (statusCode: number, headers: any, body: string) => void
) {
...
function goHead(
	...
	isValid: TestCallback,
	range: string,
	callback: (statusCode: number, headers: any) => void
) {
	starter(info.startServer(), info, isValid, content, (r, s) => {
		info.req(
			{
...
export default ((info, isValid) => {
	...
	go(info, isValid, 'bytes=0-100', (statusCode, headers, body) => {
		isValid(
			...
			'The content length returned must be the maximum length possible, but instead of ' +
				content.length +
				', got ' +
	...
	go(info, isValid, 'bytes=0-1', (statusCode, headers, body) => {
		isValid(
			...
			'The content length returned must be equals to 2 when 0-1 is asked, but instead of ' +
				2 +
				', got ' +
				headers['content-length'] +
				'.'
	...
	go(info, isValid, 'bytes=0-0', (statusCode, headers, body) => {
		isValid(
			...
			'The content length returned must be equals to 1 when 0-0 is asked, but instead of ' +
				1 +
				', got ' +
				headers['content-length'] +
				'.'
	...
	go(info, isValid, 'bytes=0-0,1-1', (statusCode, headers, body) => {
		isValid(
			/^--[^\n]+\n[^\n]+\n[^\n]+\n\r\nH\r\n--[^\n]+\n[^\n]+\n[^\n]+\n\r\ne\r\n--[^-]+--$/.test(body),
			'Expected multipart "H -- e" but got "' + body + '".'
		);
	});
	...
	go(info, isValid, 'bytes=-1,1-1', (statusCode, headers, body) => {
		isValid(
			/^--[^\n]+\n[^\n]+\n[^\n]+\n\r\n!\r\n--[^\n]+\n[^\n]+\n[^\n]+\n\r\ne\r\n--[^-]+--$/.test(body),
			'Expected multipart "! -- e" but got "' + body + '".'
		);
	});
}) as Test;
