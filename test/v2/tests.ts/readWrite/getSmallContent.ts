...
export default ((info, isValid) => {
	...
	starter(server, info, isValid, content, (r) => {
		info.req(
			...
			(res, body) => {
				isValid(
					body === content,
					'The content read is not the same as the one written : "' + body + '" but expected "' + content + '".'
				);
			}
		);
	});
	...
	starter(server2, info, isValid, content, (r) => {
		info.req(
			...
			(res) => {
				isValid(
					res.headers['content-length'] === content.length.toString(),
					'The content read is not the same as the one written : "' +
