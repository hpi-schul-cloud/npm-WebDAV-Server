			...
			(req, body) => {
				try {
					const sub = body
						...
						.map((r) => r.find('DAV:propstat').find('DAV:prop').find('DAV:displayname').findText());
					...
					isValid(
						subFiles.length === 0,
						'All children were not returned ; here are the left ones : ' + subFiles.toString()
					);
				} catch (ex) {
	...
	starter(server2, info, isValid, (r, subFiles) => {
		info.reqXML(
			...
			(req, body) => {
				try {
					isValid(
						body.find('DAV:multistatus').findMany('DAV:response').length === 1,
						'Too many or not enought DAV:response tags.'
					);
				} catch (ex) {
					isValid(false, 'Invalid WebDAV response body.', ex);
				}
