...
export default ((info, isValid) => {
				...
				(req, body, bodySource) => {
					try {
						const sub = body
							...
							.map((r) => r.find('DAV:location').find('DAV:href').findText());
						...
						for (const sf of sub) {
							...
							if (index === -1)
								return isValid(
									false,
									`Got a file name in "readDir(...)" which must not exist here with depth ${depth} : ${path}`
								);
						...
						isValid(
							allFiles.length === 0,
							`All children were not returned with depth ${depth} ; here are the left ones : ${allFiles.toString()}`
						);
					} catch (ex) {
	...
	const depthValues = ['infinity', 'InFiNiTy', -1];
