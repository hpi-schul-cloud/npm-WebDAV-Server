		...
		(req, body) => {
			try {
				...
				const invalidEntries = sub.filter((entry) => {
					if (entry.href === encodeURI(entry.href)) return false; // No need to be encoded

					return decodeURI(entry.href) === entry.href || decodeURI(entry.locationHref) === entry.locationHref;
				});

				if (invalidEntries.length === 0) return callback();

				isValid(
					false,
					'Some resource\'s url are not correctly encoded in PROPFIND in "' +
						root +
						'", displayname must not be encoded while location.href and href must be encoded : ' +
						invalidEntries
							.map(
								(entry) =>
									'displayName = "' +
