...
export default (async(info, isValid) =>
{
    ...
    const createTree = async(server) => new Promise((resolve, reject) => {
    ...
    const checkRequest = async(server, userInfo : { username : string, password : string, realm ?: string, uri ?: string }, path, httpCode) => new Promise((resolve, reject) => {
