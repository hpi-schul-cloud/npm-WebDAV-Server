...
function prob(
	...
	path: string | v2.Path,
	expectedType: v2.ResourceType,
	callback: () => void
): void {
	const ctx = server.createExternalContext();
	server.getResource(ctx, path, (e, r) => {
		if (e) return isValid(false, 'Could not find the resource ' + path, e);

		r.type((e, type) => {
			if (e) return isValid(false, 'Error with the resource ' + path, e);
			if (type !== expectedType) return isValid(false, 'Wrong type for the resource ' + path);
