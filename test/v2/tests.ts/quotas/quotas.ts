...
export default ((info, isValid) => {
	...
	starter(info, isValid, (s) => {
		propfind(s, info, 'folder', v2.HTTPCodes.MultiStatus, 0, undefined, (xml) => {
			...
			if (available !== 100) return isValid(false, 'The "DAV:quota-available-bytes" must be equals to 100');
			if (used > 0) return isValid(false, 'The "DAV:quota-used-bytes" must contains 0');
		...
		info.req(
			...
			() => {
				propfind(s, info, 'file2', v2.HTTPCodes.MultiStatus, 0, undefined, (xml) => {
					...
					if (available >= 100) return isValid(false, 'The "DAV:quota-available-bytes" must be lesser than 100');
					if (used <= 0) return isValid(false, 'The "DAV:quota-used-bytes" must contains a value greater than 0');
	...
	let content = '';
	for (let i = 0; i < 1000; ++i) content += 'A';
	...
	let content2 = '';
	for (let i = 0; i < 100; ++i) content2 += 'A';
	...
	let content5 = '';
	for (let i = 0; i < 100; ++i) content5 += 'A';
	...
	let content6 = '';
	for (let i = 0; i < 1000; ++i) content6 += 'A';
