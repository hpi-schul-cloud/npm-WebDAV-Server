...
export function proppatch(
	...
	bodySet: string[],
	bodyRemove: string[],
	callback: (xml: XMLElementUtil) => void
) {
	let body = '<D:propertyupdate xmlns:D="DAV:" xmlns:Z="http://ns.example.com/standards/z39.50/">';
	if (bodySet && bodySet.length > 0) body += '<D:set><D:prop>' + bodySet.join() + '</D:prop></D:set>';
	if (bodyRemove && bodyRemove.length > 0) body += '<D:remove><D:prop>' + bodyRemove.join() + '</D:prop></D:remove>';
