...
export function unserialize(
	...
	managers: FSManager[],
	callback: (error: Error, rootResource: IResource) => void
) {
	for (const m of managers)
		if (m.uid === obj.managerUID) {
			...
			const go = (e) => {
				...
				--nb;
				if (nb === 0) process.nextTick(() => callback(null, resource));
			};

			obj.children.forEach((c) =>
				unserialize(c, managers, (e, r) => {
					if (e) go(e);
					else resource.addChild(r, go);
				})
			);
			return;
		}
		...
		resource.getChildren((e, children) => {
			process.nextTick(() => {
				...
				function go(error, subObj) {
					...
					if (subObj) obj.children.push(subObj);
					--nb;
					if (nb === 0) process.nextTick(() => callback(null, obj));
				}
