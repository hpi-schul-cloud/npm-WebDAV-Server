...
export class PhysicalFSManager implements FSManager {
	...
	serialize(resource: any, obj: SerializedObject): object {
		if (!resource.realPath) throw Errors.UnrecognizedResource;
	...
	unserialize(data: any, obj: SerializedObject): IResource {
		...
		if (obj.type.isFile) rs = new PhysicalFile(data.realPath, null, this);
		else rs = new PhysicalFolder(data.realPath, null, this);
	...
	newResource(fullPath: string, name: string, type: ResourceType, parent: IResource): IResource {
		...
		if (!parentRealPath) throw Errors.ParentPropertiesMissing;

		const newRealPath = path.join(parentRealPath, name);

		if (type.isDirectory) return new PhysicalFolder(newRealPath, parent, this);
		else return new PhysicalFile(newRealPath, parent, this);
	}
}
