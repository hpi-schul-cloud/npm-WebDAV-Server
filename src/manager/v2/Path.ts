export class Path {
	...
	constructor(path: Path | string[] | string) {
		...
		} else if (path.constructor === Path) this.paths = (path as Path).paths.filter((x) => true);
		// clone
		else this.paths = path as string[];
	...
	getChildPath(childPath: string | Path): Path {
		...
		for (const subName of subPath.paths) path.paths.push(subName);
		return path;
	}
	...
	toString(endsWithSlash = false): string {
		...
		if (endsWithSlash && value.length > 1) return value + '/';
		return value;
	}
}
