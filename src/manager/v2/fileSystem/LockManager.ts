...
export class LocalLockManager implements ILockManager {
	...
	constructor(serializedData?: any) {
		if (serializedData) for (const name in serializedData) this[name] = serializedData[name];
	}
	...
	getLock(uuid: string, callback: ReturnCallback<Lock>): void {
		...
		for (const lock of this.locks) if (lock.uuid === uuid) return callback(null, lock);
