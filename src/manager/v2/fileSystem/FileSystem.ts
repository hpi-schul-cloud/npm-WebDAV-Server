...
export abstract class FileSystem implements ISerializableFileSystem {
				...
				ctx.server.options.storageManager.evaluateCreate(ctx, this, path, type, (size) => {
					ctx.server.options.storageManager.reserve(ctx, this, size, (reserved) => {
						...
						this._create(
							...
							(e) => {
								if (e) ctx.server.options.storageManager.reserve(ctx, this, -size, () => callback(e));
								else callback();
							}
						);
					});
				...
				this.fastExistCheckExReverse(ctx, path, callback, () => {
					this.type(ctx, path.getParent(), (e, type) => {
						if (e === Errors.ResourceNotFound) {
							if (!createIntermediates) return callback(Errors.IntermediateResourceMissing);
							...
							this.getFullPath(ctx, path, (e, fullPath) => {
								...
								ctx.getResource(fullPath, (e, r) => {
									...
									r.create(ResourceType.Directory, true, (e) => {
										if (e && e !== Errors.ResourceAlreadyExists) return callback(e);
						...
						if (!type.isDirectory) return callback(Errors.WrongParentTypeForCreation);
	...
	openWriteStreamAsync(
		...
		return promisifyCall((cb) =>
			this.openWriteStream(ctx, path, mode, targetSource, estimatedSize, (e, data1, data2) =>
	...
	openWriteStream(
		ctx: RequestContext,
		_path: Path | string,
		_mode: OpenWriteStreamMode | boolean | number | Return2Callback<Writable, boolean>,
		...
		for (const obj of [_mode, _targetSource]) if (obj && obj.constructor === Boolean) targetSource = obj as boolean;
		...
		for (const obj of [_mode, _targetSource, _estimatedSize, _callback])
			if (obj && obj.constructor === Function) callbackFinal = obj as Return2Callback<Writable, boolean>;
		...
		const callback: Return2Callback<Writable, boolean> = (e, stream, created) => {
			if (!e) this.emit('openWriteStream', ctx, path, { targetSource, mode, estimatedSize, created, stream });
			...
			() => {
				this.isLocked(ctx, path, (e, isLocked) => {
							...
							ctx.server.options.storageManager.evaluateContent(ctx, this, size, (sizeStored) => {
								if (
									...
									ctx.server.options.storageManager.available(ctx, this, (available) => {
										...
										if (available === 0) return callback(Errors.InsufficientStorage);
										...
										finalGo((e, wStream, created) => {
											...
											const stream = new Transform({
												transform(chunk, encoding, callback) {
													nb += chunk.length;
													if (nb > available) callback(Errors.InsufficientStorage);
													else callback(null, chunk, encoding);
												},
											});
											stream.pipe(wStream);
											stream.on('finish', () => {
												ctx.server.options.storageManager.reserve(ctx, this, nb, (reserved) => {
													if (!reserved) stream.emit('error', Errors.InsufficientStorage);
												});
											});
					...
					switch (mode) {
						...
						case 'canCreate':
							go((e, wStream) => {
								if (e === Errors.ResourceNotFound) createAndGo(mode === 'canCreateIntermediates');
	...
	openReadStream(
		...
		const targetSource = ensureValue(
			_targetSource.constructor === Boolean ? (_targetSource as boolean) : undefined,
			false
		);
		const estimatedSize = ensureValue(
			_callback ? (_estimatedSize as number) : _estimatedSize ? (_targetSource as number) : undefined,
			-1
		);
		...
		const callback: ReturnCallback<Readable> = (e, stream) => {
			if (!e) this.emit('openReadStream', ctx, path, { targetSource, estimatedSize, stream });
	...
	move(
		...
		const callback: ReturnCallback<boolean> = (e, overrided) => {
			if (!e) this.emit('move', ctx, pathFrom, { pathFrom, pathTo, overwrite, overrided });
				...
				this.isLocked(ctx, pathFrom, (e, isLocked) => {
					...
					this.isLocked(ctx, pathTo, (e, isLocked) => {
						...
						this.fastExistCheckEx(ctx, pathFrom, callback, () => {
							if (!overwrite) this.fastExistCheckExReverse(ctx, pathTo, callback, go);
							else go();
						});
					});
				});
	...
	copy(
		...
		const callback: ReturnCallback<boolean> = (e, overrided) => {
			if (!e) this.emit('copy', ctx, pathFrom, { pathTo, overwrite, overrided, depth });
			callbackFinal(e, overrided);
		};
							...
							ctx.server.getFileSystem(newPath, (fs, _, subPath) => {
								const go = (overwritten: boolean) => {
									ctx.server.setFileSystem(newPath, this, (successed) => {
										if (!successed) return callback(Errors.InvalidOperation);
						...
						setLock(lock: Lock, callback: SimpleCallback): void {
							...
							issuePrivilegeCheck(this, ctx, pPath, 'canWriteLocks', callback, () => {
								buffIsLocked.isLocked((e, isLocked) => {
									...
									lm.setLock(lock, (e) => {
										if (!e) this.emit('lock-set', ctx, pPath, { lock });
										callback(e);
									});
								});
							});
							...
							issuePrivilegeCheck(this, ctx, pPath, 'canWriteLocks', callback, () => {
								buffIsLocked.isLocked((e, isLocked) => {
									...
									lm.removeLock(uuid, (e, removed) => {
										if (!e) this.emit('lock-remove', ctx, pPath, { uuid, removed });
										callback(e, removed);
									});
								});
							});
							...
							issuePrivilegeCheck(this, ctx, pPath, 'canWriteLocks', callback, () => {
								buffIsLocked.isLocked((e, isLocked) => {
									...
									lm.refresh(uuid, timeoutSeconds, (e, lock) => {
										if (!e) this.emit('lock-refresh', ctx, pPath, { uuid, timeout: timeoutSeconds, lock });
										callback(e, lock);
									});
								});
							});
							...
							issuePrivilegeCheck(this, ctx, pPath, 'canWriteProperties', callback, () => {
								buffIsLocked.isLocked((e, isLocked) => {
									...
									pm.setProperty(name, value, attributes, (e) => {
										if (!e) this.emit('property-set', ctx, pPath, { name, value, attributes });
										callback(e);
									});
								});
							});
							...
							issuePrivilegeCheck(this, ctx, pPath, 'canWriteProperties', callback, () => {
								buffIsLocked.isLocked((e, isLocked) => {
									...
									pm.removeProperty(name, (e) => {
										if (!e) this.emit('property-remove', ctx, pPath, { name });
										callback(e);
									});
								});
							});
	...
	creationDate(ctx: RequestContext, path: Path | string, callback: ReturnCallback<number>): void {
		...
		issuePrivilegeCheck(this, ctx, pPath, 'canReadProperties', callback, () => {
			this.fastExistCheckEx(ctx, pPath, callback, () => {
				if (!this._creationDate && !this._lastModifiedDate) return callback(null, 0);
				if (!this._creationDate) return this.lastModifiedDate(ctx, pPath, callback);
	...
	lastModifiedDate(ctx: RequestContext, path: Path | string, callback: ReturnCallback<number>): void {
		...
		issuePrivilegeCheck(this, ctx, pPath, 'canReadProperties', callback, () => {
			this.fastExistCheckEx(ctx, pPath, callback, () => {
				if (!this._creationDate && !this._lastModifiedDate) return callback(null, 0);
				if (!this._lastModifiedDate) return this.creationDate(ctx, pPath, callback);
	...
	webName(ctx: RequestContext, path: Path | string, callback: ReturnCallback<string>): void {
		...
		issuePrivilegeCheck(this, ctx, pPath, 'canReadProperties', callback, () => {
			this.fastExistCheckEx(ctx, pPath, callback, () => {
				if (pPath.isRoot()) this.getFullPath(ctx, (e, pPath) => callback(e, e ? null : pPath.fileName()));
	...
	addSubTree(
		ctx: RequestContext,
		_rootPath: Path | SubTree | ResourceType | SimpleCallback | string | Buffer,
	...
	listDeepLocks(
		...
		this.lockManager(ctx, pStartPath, (e, lm) => {
			...
			lm.getLocks((e, locks) => {
				...
				if (depth !== -1) locks = locks.filter((f) => f.depth === -1 || f.depth >= depth);
				...
				const go = (fs: FileSystem, parentPath: Path) => {
					...
					fs.listDeepLocks(ctx, parentPath, destDepth, (e, pLocks) => {
						...
						if (locks && locks.length > 0) pLocks[pStartPath.toString()] = locks;
				...
				if (!pStartPath.isRoot()) {
					...
					this.getFullPath(ctx, (e, fsPath) => {
						...
						if (fsPath.isRoot()) {
							const result = {};
							if (locks && locks.length > 0) result[pStartPath.toString()] = locks;
	...
	privilegeManager(ctx: RequestContext, path: Path | string, callback: ReturnCallback<PrivilegeManager>) {
		if (!this._privilegeManager) return callback(null, ctx.server.options.privilegeManager);
