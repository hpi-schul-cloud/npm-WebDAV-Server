		...
		ctx.getResource((e, r) => {
			ctx.checkIfHeader(r, () => {
				...
				const cb = (e?: Error, overwritten?: boolean) => {
					if (e) {
						if (e === Errors.ResourceAlreadyExists) ctx.setCode(HTTPCodes.PreconditionFailed);
						else if (!ctx.setCodeFromError(e)) ctx.setCode(HTTPCodes.InternalServerError);
				...
				ctx.server.getFileSystem(destination, (destFs, destRootPath, destSubPath) => {
					if (destFs !== r.fs) {
						...
						else StandardMethods.standardCopy(ctx, r.path, r.fs, destSubPath, destFs, overwrite, cb);
					} else {
