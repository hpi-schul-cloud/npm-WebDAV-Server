...
export default class implements HTTPMethod {
	addXMLInfo(
			...
			if (e === Errors.MustIgnore || e === Errors.ResourceNotFound) e = null;
				...
				if (errorNumber !== null) {
					...
					resource.fs.getFullPath(ctx, resource.path, (e, path) => {
						...
						if (ctx.server.options.enableLocationTag) response.ele('D:location').ele('D:href', undefined, true).add(p);
					});
		...
		function displayValue(values: string[] | string, fn: () => void) {
			if (
				values.constructor === String ? tags[values as string].value : (values as string[]).some((n) => tags[n].value)
			) {
				++nb;
				process.nextTick(fn);
			}
		...
		displayValue('getlastmodified', () => {
			resource.lastModifiedDate((e, lastModifiedDate) =>
				process.nextTick(() => {
					if (!e && tags.getlastmodified.value) tags.getlastmodified.el.add(new Date(lastModifiedDate).toUTCString());
					nbOut(e);
				})
			);
		});
					...
					for (const name in properties) {
						if (reqBody.mustDisplay(name)) {
							...
							if (reqBody.mustDisplayValue(name)) {
								...
								if (tag.attributes)
									for (const attName in property.attributes) tag.attributes[attName] = property.attributes[attName];
		...
		ctx.getResource((e, resource) => {
			ctx.checkIfHeader(resource, () => {
				...
				resource.type((e, type) =>
					process.nextTick(() => {
						if (e) {
							if (!ctx.setCodeFromError(e)) ctx.setCode(HTTPCodes.InternalServerError);
							return callback();
						}
