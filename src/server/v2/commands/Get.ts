...
class RangedStream extends Transform {
	...
	_transform(chunk: string | Buffer, encoding: string, callback: (_: any, chunk: string | Buffer) => void) {
		if (this.nb < this.min) {
			...
			this.nb += chunk.length;
			if (this.nb > this.max) chunk = chunk.slice(0, this.max - (this.nb - chunk.length) + 1);
			callback(null, chunk);
		}
	}
}
									...
									process.nextTick(() => {
										...
										r.etag((e, etag) => {
											...
											r.openReadStream(targetSource, (e, rstream) => {
												if (e) {
													if (!ctx.setCodeFromError(e)) ctx.setCode(HTTPCodes.MethodNotAllowed);
													return callback();
												}
												...
												if (range) {
													try {
														const { ranges, separator, len, createMultipart, endMultipart } = parseRangeHeader(
															mimeType,
															size,
															range
														);
