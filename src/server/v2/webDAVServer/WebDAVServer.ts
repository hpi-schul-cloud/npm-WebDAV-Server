...
export class WebDAVServer {
	...
	protected isSameFileSystem(fs: FileSystem, path: string, checkByReference: boolean): boolean {
		return (
			...
			(!checkByReference && this.fileSystems[path].serializer().uid() === fs.serializer().uid())
		);
	}
	...
	getFileSystemPathSync(fs: FileSystem, checkByReference?: boolean): Path {
		...
		for (const path in this.fileSystems) if (this.isSameFileSystem(fs, path, checkByReference)) return new Path(path);
		return null;
	}
	...
	getFileSystemAsync(path: Path): Promise<{ fs: FileSystem; rootPath: Path; subPath: Path }> {
		return promisifyCall((cb) =>
			this.getFileSystem(path, (fs, rootPath, subPath) => cb(undefined, { fs, rootPath, subPath }))
		);
	}
	...
	emit(event: string, ctx: RequestContext, fs: FileSystem, path: Path | string, data?: any): void {
		...
		this.events[event].forEach((l) =>
			process.nextTick(() => l(ctx, fs, path.constructor === String ? new Path(path as string) : (path as Path), data))
		);
	}
