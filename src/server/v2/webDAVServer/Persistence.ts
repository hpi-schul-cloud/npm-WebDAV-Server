import {
	...
	unserialize,
	SerializedData,
} from '../../../manager/v2/fileSystem/Serialization';
...
export function autoLoad(callback: SimpleCallback) {
	...
	if (!streamProvider) streamProvider = (s, cb) => cb(s.pipe(zlib.createGunzip()));
...
function loadDefaultIAutoSaveParam(options: IAutoSave) {
	...
	if (!options.tempTreeFilePath) options.tempTreeFilePath = options.treeFilePath + '.tmp';
