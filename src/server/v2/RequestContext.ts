...
export class ExternalRequestContext extends RequestContext {
	...
	static create(
		server: WebDAVServer,
		_options?: RequestContextExternalOptions | ((error: Error, ctx: ExternalRequestContext) => void),
		_callback?: (error: Error, ctx: ExternalRequestContext) => void
	): ExternalRequestContext {
		const defaultValues = new DefaultRequestContextExternalOptions();

		const options =
			_options && _options.constructor !== Function ? (_options as RequestContextExternalOptions) : defaultValues;
		...
		if (defaultValues !== options) {
			for (const name in defaultValues) if (options[name] === undefined) options[name] = defaultValues[name];
		}
...
export class HTTPRequestContext extends RequestContext {
	...
	static create(
		...
		const setAllowHeader = (type?: ResourceType) => {
			...
			for (const name in server.methods) {
				...
				if (!method.isValidFor || method.isValidFor(ctx, type)) allowedMethods.push(name.toUpperCase());
			}
	...
	writeBody(xmlObject: XMLElement | any) {
