...
class RangedStream extends Transform {
	...
	_transform(chunk: any, encoding: string, callback: Function) {
		if (this.nb < this.min) {
			...
			this.nb += chunk.length;
			if (this.nb > this.max) chunk = chunk.slice(0, this.max - (this.nb - chunk.length));
			callback(null, chunk);
		}
	}
}
	...
	arg.noBodyExpected(() => {
		arg.getResource((e, r) => {
			...
			arg.checkIfHeader(r, () => {
				...
				arg.requirePrivilege(
					targetSource ? ['canRead', 'canSource', 'canGetMimeType'] : ['canRead', 'canGetMimeType'],
					r,
					() => {
						r.type((e, type) => {
							if (e) {
