...
function lockDiscovery(
	...
	callback = (e, l) => {
		if (!e) lockDiscoveryCache[path.toString()] = l;
		_Callback(e, l);
	};
...
function parseRequestBody(arg: MethodCallArgs): PropertyRule {
	...
	if (arg.contentLength <= 0) return allTrue;
	...
	arg.getResource((e, resource) => {
		...
		arg.checkIfHeader(resource, () => {
					...
					if (!type.isDirectory || arg.depth === 0) {
						addXMLInfo(resource, multistatus, (e) => {
							if (!e) done(multistatus);
								...
								if (e === Errors.BadAuthentication) arg.setCode(HTTPCodes.Unauthorized);
								else arg.setCode(HTTPCodes.InternalServerError);
								callback();
							}
						});
						return;
			...
			function addXMLInfo(resource: IResource, multistatus, _callback) {
				...
				if (targetSource) privileges.push('canSource');
				arg.requireErPrivilege(privileges, resource, (e, can) => {
					...
					function nbOut(error?) {
						...
						if (nb === 0) {
							if (reqBody.leftElements.length > 0) {
								...
								for (const el of reqBody.leftElements) if (el) prop.ele(el.name);
							}
							callback();
						}
					}
					...
					displayValue('creationdate', () => {
						resource.creationDate((e, ticks) =>
							process.nextTick(() => {
								if (!e) tags.creationdate.el.add(arg.dateISO8601(ticks));
								nbOut(e);
							})
						);
					});
					...
					arg.getResourcePath(resource, (e, path) => {
						...
						resource.type((e, type) =>
							process.nextTick(() => {
								...
								if (tags.resourcetype.value && type.isDirectory) tags.resourcetype.el.ele('D:collection');
											...
											process.nextTick(() => {
												if (!e)
													tags.getcontentlength.el.add(
														size === undefined || size === null || size.constructor !== Number ? 0 : size
													);
												nbOut(e);
											})
										);
					...
					displayValue('displayname', () => {
						...
						methodDisplayName.bind(resource)((e, name) =>
							process.nextTick(() => {
								if (!e) tags.displayname.el.add(name ? encode(name) : '');
								nbOut(e);
							})
						);
					});
					...
					displayValue(['getetag', 'getlastmodified'], () => {
						resource.lastModifiedDate((e, lastModifiedDate) =>
							process.nextTick(() => {
								if (!e) {
									if (tags.getetag.value) tags.getetag.el.add(ETag.createETag(lastModifiedDate));
									if (tags.getlastmodified.value) tags.getlastmodified.el.add(new Date(lastModifiedDate).toUTCString());
								}
								nbOut(e);
							})
						);
							...
							process.nextTick(() => {
								...
								for (const name in properties) {
									if (reqBody.mustDisplay(name)) {
										...
										if (reqBody.mustDisplayValue(name)) tag.add(properties[name]);
									}
								}
								nbOut();
							})
