...
function copy(arg: MethodCallArgs, source: IResource, rDest: IResource, destination: FSPath, callback: SimpleCallback) {
	...
	function _(error: Error, cb) {
		if (error) process.nextTick(() => callback(error));
					...
					arg.requirePrivilege(['canCreate', 'canSetProperty', 'canWrite'], dest, () => {
						dest.create((e) =>
							_(e, () => {
								...
								rDest.addChild(dest, (e) =>
									_(e, () => {
										arg.invokeEvent('addChild', rDest, dest);

										copyAllProperties(source, dest, (e) =>
											_(e, () => {
												if (!type.isFile) {
													next();
													return;
															...
															new Workflow()
																.each(children, (child, cb) => {
																	child.webName((e, name) =>
																		process.nextTick(() => {
																			if (e) cb(e);
																			else copy(arg, child, dest, destination.getChildPath(name), cb);
																		})
																	);
																})
																.error(callback)
								...
								process.nextTick(() => {
									...
									function done(overridded: boolean) {
										copy(arg, source, rDest, destination, (e) => {
											if (e) arg.setCode(HTTPCodes.InternalServerError);
											else if (overridded) arg.setCode(HTTPCodes.NoContent);
											else arg.setCode(HTTPCodes.Created);
											callback();
										});
									}
