...
export class WebDAVServer {
	...
	constructor(options?: WebDAVServerOptions) {
		...
		for (const k in Commands)
			if (k === 'NotImplemented') this.onUnknownMethod(Commands[k]);
			else this.method(k, Commands[k]);
	}
	...
	getResourceFromPath(
		arg: MethodCallArgs,
		path: FSPath | string[] | string,
		callbackOrRootResource: ReturnCallback<IResource> | IResource,
		callback?: ReturnCallback<IResource>
	) {
	...
	addResourceTree(
		_rootResource: IResource | ResourceTreeNode,
		_resoureceTree: ResourceTreeNode | ((e: Error) => void),
	...
	// Events
	invoke(
		event: events.EventsName,
		arg: MethodCallArgs,
		subjectResource?: IResource | FSPath,
		details?: events.DetailsType
	) {
	...
	on(event: events.EventsName, eName_listener: string | events.Listener, listener?: events.Listener) {
		if (eName_listener.constructor === Function) events.register.bind(this)(event, eName_listener);
		else events.registerWithName.bind(this)(event, eName_listener, listener);
	}
