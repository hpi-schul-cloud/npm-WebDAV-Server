...
export function getResourceFromPath(
	...
	} else rootResource = callbackOrRootResource;

	let paths: FSPath;
	if (path.constructor === FSPath) paths = path as FSPath;
	else paths = new FSPath(path);
	...
	rootResource.getChildren((e, children) => {
		...
		function done() {
			--nb;
			if (nb === 0 && !found) process.nextTick(() => callback(Errors.ResourceNotFound, null));
		}
...
export function addResourceTree(
	_rootResource: IResource | ResourceTreeNode,
	_resoureceTree: ResourceTreeNode | (() => void),
