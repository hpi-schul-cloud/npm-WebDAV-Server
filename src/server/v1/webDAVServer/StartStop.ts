...
function autoSave(options: IAutoSave) {
	if (!options.streamProvider) options.streamProvider = (s, cb) => cb(s);
...
export function start(port?: number | WebDAVServerStartCallback, callback?: WebDAVServerStartCallback) {
	...
	if (port && port.constructor === Number) {
		...
		if (callback) {
			if (callback instanceof Function) _callback = callback;
			else throw Errors.IllegalArguments;
		}
	} else if (port && port.constructor === Function) {
		_port = this.options.port;
		_callback = port as WebDAVServerStartCallback;
		if (callback) throw Errors.IllegalArguments;
	}
	...
	if (!this.server) {
			...
			MethodCallArgs.create(this, req, res, (e, base) => {
				...
				if (!this.options.canChunk || !method.chunked) {
					...
					if (base.contentLength <= 0) {
						...
						let index = 0;
						req.on('data', (chunk) => {
							if (chunk.constructor === String) chunk = Buffer.from(chunk as string);

							for (let i = 0; i < chunk.length && index < data.length; ++i, ++index) data[index] = (chunk as Buffer)[i];
		...
		if (this.options.autoSave) autoSave.bind(this)(this.options.autoSave);
	}
