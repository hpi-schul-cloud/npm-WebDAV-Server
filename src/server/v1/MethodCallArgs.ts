...
export class MethodCallArgs {
	...
	protected constructor(
		...
		this.isSource =
			this.findHeader('source', 'F').toUpperCase() === 'T' || this.findHeader('translate', 'T').toUpperCase() === 'F';
	...
	static create(
		...
		function setAllowHeader(type?: ResourceType) {
			...
			for (const name in server.methods) {
				...
				if (!method.isValidFor || method.isValidFor(type)) allowedMethods.push(name.toUpperCase());
			}
	...
	requireErCustomPrivilege(
		privileges: string | string[],
		resource: IResource,
		callback: (error: Error, can: boolean) => void
	) {
	...
	accept(regex: RegExp[]): number {
		...
		for (const value of accepts) {
			for (let i = 0; i < regex.length; ++i) if (regex[i].test(value)) return i;
		}
	...
	findHeader(name: string, defaultValue: string = null): string {
		...
		for (const k in this.request.headers)
			if (k.replace(/(-| )/g, '').toLowerCase() === name) return this.request.headers[k].toString();
	...
	wrapEvent(event: EventsName, subjectResource?: IResource, details?: DetailsType) {
		...
		this.exit = () => {
			if (Math.floor(this.response.statusCode / 100) === 2) this.invokeEvent(event, subjectResource, details);
	...
	getResourcePath(resource: IResource, callback: ReturnCallback<string>) {
		if (!resource.parent) callback(null, '/');
		else
