...
export class Privilege implements IPrivilege {
	...
	static readCurrentUserPrivilegeSet = new Privilege(
		'DAV:read-current-user-privilege-set',
		'Read current user privilege set property',
		true
	);
	static read = new Privilege('DAV:read', 'Read any object', false, [
		Privilege.readACL,
	...
	static write = new Privilege('DAV:write', 'Write any object', false, [
		Privilege.writeACL,
	...
	constructor(
		...
		description: string,
		isAbstract: boolean,
		subPrivileges?: (string | Privilege)[]
	);
	...
	constructor(
		...
		description: string,
		isAbstract: boolean,
		subPrivileges?: (string | Privilege)[] | boolean
	) {
		this.description = description;
		this.isAbstract = isAbstract;
		this.isAll = subPrivileges && subPrivileges.constructor === Boolean ? (subPrivileges as boolean) : false;

		if (!subPrivileges || subPrivileges.constructor !== Array) subPrivileges = [];
		const subPrivilegesCast = subPrivileges as (string | Privilege)[];

		if (name.constructor === Privilege) this.name = (name as Privilege).name;
		else this.name = name as string;

		subPrivilegesCast.push(this.name);
		this.subPrivileges = subPrivilegesCast.map((p) =>
			p.constructor === Privilege ? (p as Privilege).name : (p as string)
		);
	}
