...
export class HTTPDigestAuthentication implements HTTPAuthentication {
	...
	getUser(arg: MethodCallArgs, userManager: IUserManager, callback: (error: Error, user: IUser) => void) {
		...
		userManager.getUserByName(authProps.username, (e, user) => {
			...
			if (authProps.algorithm === 'MD5-sess') ha1 = md5(ha1 + ':' + authProps.nonce + ':' + authProps.cnonce);

			let ha2;
			if (authProps.qop === 'auth-int') return onError(Errors.WrongHeaderFormat);
			// ha2 = md5(ctx.request.method.toString().toUpperCase() + ':' + ctx.requested.uri + ':' + md5(...));
			else ha2 = md5(arg.request.method.toString().toUpperCase() + ':' + arg.uri);
			...
			if (result.toLowerCase() === authProps.response.toLowerCase()) callback(Errors.None, user);
