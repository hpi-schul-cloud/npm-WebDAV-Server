...
export class PrivilegeManager {
	...
	can(
		...
		if (resource.context.overridePrivileges || (user && user.isAdministrator)) return callback(null, true);
		...
		if (this._can) return this._can(fullPath, user, resource, privilege, callback);
	...
	protected canWriteContent(fullPath: Path, user: IUser, resource: Resource, callback: PrivilegeManagerCallback): void {
		checkAll(this, [this.canWriteContentSource, this.canWriteContentTranslated], fullPath, user, resource, callback);
	}
	...
	protected canReadContent(fullPath: Path, user: IUser, resource: Resource, callback: PrivilegeManagerCallback): void {
		checkAll(this, [this.canReadContentSource, this.canReadContentTranslated], fullPath, user, resource, callback);
	}
