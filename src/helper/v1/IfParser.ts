...
function NoLock() {
	return function (r: IResource, callback: FnReturn) {
		r.getLocks((e, locks) => {
			if (e === Errors.MustIgnore) callback(null, true);
			else callback(e, locks ? locks.length === 0 : false);
		});
	};
}
...
function Tag(tag: string) {
	return function (r: IResource, callback: FnReturn) {
		r.lastModifiedDate((e, lastModifiedDate) => {
			if (e === Errors.MustIgnore) callback(null, true);
			else callback(e, !e && ETag.createETag(lastModifiedDate) === tag);
		});
	};
}
...
export function parseIfHeader(ifHeader: string) {
	...
	return function (arg: MethodCallArgs, r: IResource, callback: ReturnCallback<boolean>) {
		...
		orArray.forEach((a) => {
			if (!a.path) a.actions(r, done);
			else
				arg.server.getResourceFromPath(arg, a.path, (e, resource) => {
					if (e) done(e, null);
					else a.actions(resource, done);
				});
		});
	};
}
