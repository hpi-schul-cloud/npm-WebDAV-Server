...
export class PhysicalGateway extends PhysicalFolder {
	...
	webName(callback: ReturnCallback<string>) {
		if (this.customName) callback(null, this.customName);
		else super.webName(callback);
	}

	protected listChildren(
		parent: PhysicalResource,
		rpath: string,
		callback: (error: Error, children?: IResource[]) => void
	) {
		if (rpath.lastIndexOf('/') !== rpath.length - 1) rpath += '/';
			...
			new Workflow()
				.each(list, (file, cb) => {
					...
					fs.stat(realPath, (e, stat) => {
						...
						if (stat.isFile()) resource = new PhysicalFile(realPath, parent, this.fsManager);
						else resource = new PhysicalFolder(realPath, parent, this.fsManager);
				...
				parent.getChildren((e, actualChildren) => {
					...
					this.listChildren(parent, parentPath.toString(), (e, children) => {
						...
						actualChildren
							.filter(
								(c) =>
									c.constructor !== PhysicalResource &&
									c.constructor !== PhysicalFile &&
									c.constructor !== PhysicalFolder
							)
			...
			this.listChildren(r, path.toString(), (e, children) => {
				if (!e) {
					(r as PhysicalFolder).children.children
						.filter(
							(c) =>
								c.constructor !== PhysicalResource && c.constructor !== PhysicalFile && c.constructor !== PhysicalFolder
						)
