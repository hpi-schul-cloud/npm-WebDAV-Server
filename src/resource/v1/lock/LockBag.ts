...
export class LockBag {
	...
	getLocks(lockType?: LockType): Lock[] {
		this.cleanLocks();
		if (lockType) return this.locks.filter((l) => l.lockKind.type.isSame(lockType));
		else return this.locks;
	}
	getLock(uuid: string): Lock {
		for (const lock of this.locks) if (lock.uuid === uuid) return lock;

		return null;
	}

	setLock(lock: Lock): boolean {
		if (!this.canLock(lock.lockKind)) return false;
	...
	canLock(lockKind: LockKind): boolean {
		this.cleanLocks();
		return (
			!(lockKind.scope.isSame(LockScope.Exclusive) && this.locks.length > 0) &&
