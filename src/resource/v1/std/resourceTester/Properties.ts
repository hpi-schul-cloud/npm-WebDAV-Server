							...
							callback(e, !e, 'setProperty error', undefined, () => {
								r1.getProperties((e, props) => {
									callback(e, !e && !!props, 'getProperties error', undefined, () => {
										const valid = {};
										for (const key of keys) valid[key] = false;

										for (const name of Object.keys(props))
											if (values[name] !== undefined) valid[name] = values[name] === props[name];

										callback(
											null,
											keys.every((k) => valid[k]),
											'One or many properties are invalid or missing in the response of getProperties'
										);
									});
								});
							});
