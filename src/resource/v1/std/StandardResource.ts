...
export abstract class StandardResource implements IResource {
	...
	getAvailableLocks(callback: ReturnCallback<LockKind[]>) {
		callback(null, [new LockKind(LockScope.Exclusive, LockType.Write), new LockKind(LockScope.Shared, LockType.Write)]);
	}
	...
	getProperty(name: string, callback: ReturnCallback<ResourcePropertyValue>) {
		...
		if (value === undefined) callback(Errors.PropertyNotFound, null);
	...
	public static standardFindChildren(
		parent: IResource,
		predicate: (resource: IResource, callback: (error: Error, isMatching?: boolean) => void) => void,
		callback: ReturnCallback<IResource[]>
	) {
		parent.getChildren((e, children) => {
			...
			new Workflow()
				...
				.done((conflictingChildren) =>
					callback(
						null,
						conflictingChildren.filter((c) => !!c)
					)
				);
		});
	}
	public static standardFindChildByName(parent: IResource, name: string, callback: ReturnCallback<IResource>) {
		this.standardFindChildren(
			parent,
			(r, cb) =>
				r.webName((e, rname) => {
					if (e) cb(e);
					else if (name === rname) cb(null, true);
					else cb(null, false);
				}),
			(e, rs) => {
				if (e) callback(e, null);
				else if (rs.length > 0) callback(null, rs[0]);
				else callback(Errors.ResourceNotFound, null);
			}
		);
	}
	public static standardMoveByCopy(
		resource: IResource,
		parent: IResource,
		newName: string,
		overwrite: boolean,
		...
		StandardResource.standardFindChildByName(parent, newName, (e, r) => {
			if (e === Errors.ResourceNotFound) copy();
			else if (e) callback(e, null);
			else if (!overwrite) callback(Errors.ResourceAlreadyExists, null);
			else
				r.delete((e) => {
					if (e) callback(e, null);
					else copy();
			...
			resource.type((e, type) => {
				...
				destination.create((e) => {
					...
					parent.addChild(destination, (e) => {
						...
						if (type.isDirectory) copyDir(destination);
						else copyFile(destination);
					});
				});
			});
		}
			...
			copyProperties(destination, (e) => {
				if (e) callback(e, null);
					...
					copyLocks(destination, (e) => {
						if (e) callback(e, null);
						else if (deleteSource) resource.delete((e) => callback(e, destination));
						else resource.parent.removeChild(resource, (e) => callback(e, destination));
					});
			});
		}
				...
				new Workflow()
					.each(children, (child, cb) =>
						child.webName((e, name) => {
							if (e) cb(e);
							else child.moveTo(destination, name, overwrite, cb);
						})
					)
					.error((e) => callback(e, null))
	 ...
	 */
	public static standardMoveWithoutCopy(
		resource: IResource,
		parent: IResource,
		newName: string,
		overwrite: boolean,
		...
		StandardResource.standardFindChildByName(parent, newName, (e, r) => {
			if (e === Errors.ResourceNotFound) move();
			else if (e) callback(e);
			else if (!overwrite) callback(Errors.ResourceAlreadyExists);
			else
				r.delete((e) => {
					if (e) callback(e);
					else move();
	...
	public static standardMimeType(
		...
		if (_defaultMimeType.constructor === Function) {
			...
			if (_defaultMimeType !== undefined && _defaultMimeType !== null) useWebName = _defaultMimeType as boolean;
		} else {
			callback = _callback as ReturnCallback<string>;
			if (_useWebName !== undefined && _useWebName !== null) useWebName = _useWebName as boolean;
			if (_defaultMimeType !== undefined && _defaultMimeType !== null) defaultMimeType = _defaultMimeType as string;
		}
